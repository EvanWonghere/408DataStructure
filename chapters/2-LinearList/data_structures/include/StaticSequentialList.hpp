...
public:
     ...
     */
    bool insert(int pos, type_t val);
...
StaticSequentialList<type_t>::StaticSequentialList(int capacity)
    ...
    data = std::make_unique<type_t[]>(capacity);
}
...
template<class type_t>
int StaticSequentialList<type_t>::locate(type_t value) {
...
template<class type_t>
const type_t& StaticSequentialList<type_t>::operator[](int i) const {
    if (i < 0 || i >= length_) throw std::out_of_range("Index out of range");
    return data[i];
}

template<class type_t>
type_t& StaticSequentialList<type_t>::operator[](int i) {
    if (i < 0 || i >= length_) throw std::out_of_range("Index out of range");
    return data[i];
}

template<class type_t>
bool StaticSequentialList<type_t>::insert(int pos, type_t val) {
...
template<class type_t>
bool StaticSequentialList<type_t>::push_back(type_t val) {
    return insert(length_, val);
}

template<class type_t>
bool StaticSequentialList<type_t>::del(int pos) {
